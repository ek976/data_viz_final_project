...
dat = d3.csv('/parallel_coordinates_plot/parallel_coordinates_df.csv', function(d){
  return {
    'Country': d['Country'], 
    'Region': d['Region'], 
...
chart = async () => {
  // Load primary data
  const primaryData = await FileAttachment("report/primary_1.csv").text().then(parseCSV);

  // Load secondary data
  const secondaryData = await FileAttachment("report/secondary_1.csv").text().then(parseCSV);

  // Load tertiary data
  const tertiaryData = await FileAttachment("report/tertiary_1.csv").text().then(parseCSV);

...
df <- read[((read$Indicator.Name == "With advanced education, female") | (read$Indicator.Name == "With advanced education, male") | (read$Indicator.Name == "With basic education, female") |
(read$Indicator.Name == "With basic education, male")),]
df <- na.omit(df)
...
ggsave("line_plot.png", facet_plot, width = 6, height = 4)
```
```{r}
...
labor <- ggplot(income_group_count, aes(x = Income_Group, y = Count, fill = Income_Group)) +
  ...
  labs(title = "Countries with Labor Force Statistics Collected",
       x = "Income Group",
